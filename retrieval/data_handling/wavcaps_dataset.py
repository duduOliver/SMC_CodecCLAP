...
def _load_json_file(files, data_dir, blacklist=None):
    json_data = []
    audio_id = 0
    ...
    for file in files:
        if "FreeSound" in file:
            data_dir_sub = data_dir + "/FreeSound_flac/"
        elif "AudioSet" in file:
            data_dir_sub = data_dir + "/AudioSet_SL_flac/"
        elif "BBC_Sound_Effects" in file:
            data_dir_sub = data_dir + "/BBC_Sound_Effects_flac/"
        elif "SoundBible" in file:
            data_dir_sub = data_dir + "/SoundBible_flac/"
        with open(file, "r") as f:
            ...
            if json_obj["num_captions_per_audio"] == 1:
                for item in json_obj["data"]:
                    ...
                    if "AudioSet" in file:
                        file_name = item["id"].split('.')[0] + ".flac"    
                    else:
                        file_name = item["id"] + ".flac"          
                    temp_dict = {"audio": data_dir_sub + file_name, "caption": item["caption"], "id": audio_id,
...
class AudioLanguagePretrainDataset(Dataset):
    ...
    def __init__(self, json_files, audio_config, data_dir, blacklist=None):

        self.json_data = _load_json_file(json_files, data_dir, blacklist)
...
def wavcaps_dataloader(config,
                        bucket: bool = True,
                        bucket_boundaries: tuple = (5, 30, 6),
                        is_distributed: bool = False,
                        num_tasks: int = 0,
                        global_rank: int = 0):
    dataset = AudioLanguagePretrainDataset(config["json_files"], config["audio_args"], config["data_dir"], config["blacklist"])
    if bucket:
