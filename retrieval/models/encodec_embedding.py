# below codes are based and adapted from https://github.com/jhurliman/music-interpolation
...
class EncodecEmbedding:
    """
    This uses the SEANet model only to convert audio into a latent embedding (continuous vector).
    """
    ...
    def embedding(
        self, audio_a: NDFloat | None = None
    ) -> NDFloat:
        ...
        inputs_a = cast(
            ProcessedInput,
            self.processor(
                raw_audio=audio_a.tolist(), sampling_rate=self.sampling_rate, return_tensors="pt"
            ),
        )
        padding_mask = inputs_a["padding_mask"]
        # print("inputs_a[\"input_values\"] type is ", type(inputs_a["input_values"]) )
        # print("padding_mask type is ", type(padding_mask) )
        # print("inputs_a[\"input_values\"] shape after self.processor: ", inputs_a["input_values"].shape)
        # print("inputs_a[\"padding_mask\"] shape after self.processor: ", inputs_a["padding_mask"].shape)
        embedding_a, scale_a = encode_embedding(
            inputs_a["input_values"].to(self.model.device), self.model.encoder, self.model.config, padding_mask[:,None,:].repeat(1, 2, 1).to(self.model.device)
        )
        
        # print("embedding_a shape: ", embedding_a.shape)
...
def encode_embedding(
    ...
    # print("input_values shape in encode_embedding: ", input_values.shape)
    ...
    for offset in range(0, input_length - step, stride):
        ...
        # print("mask shape in encode_embedding: ", mask.shape)
        frame = input_values[:, :, offset : offset + chunk_length]
        # print("frame shape in encode_embedding: ", frame.shape)
        embeddings, scale = _encode_frame_embeddings(frame, mask, encoder, config)
        frame_embeddings.append(embeddings)
        scales.append(scale)

    return torch.stack(frame_embeddings), torch.stack(scales)
...
def _encode_frame_embeddings(
    ...
    # print("input_values shape in _encode_frame_embeddings: ", input_values.shape)
